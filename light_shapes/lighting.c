...
void judge(int *poly, double x, double y, double z, int *gb, double *view) {
  double cx, cy, cz;
  int l=poly[0], i;
  for(i=1; i<=l; i++) {
    cx+=x[poly[i]];
  ...
  int good = view[0]*cx + view[1]*cy + view[2]*cx + view[3];
  int side;
  for(i=i;i<=l;i++) {
    side = view[0]*x[poly[i]] 
      + view[1]*y[poly[i]] 
      + view[2]*z[poly[i]] 
      + view[3];
    if(side<=0 && good <= 0) gb[i] = 1;
    else if(side >= 0 && good >= 0) gb[i] = 1;
    else gb[i] = 0;
  }
}
void clip(int *poly, double *x, double *y, double *z) {
  int l = poly[0];
  int i,j, nj, k;
  int gb[l+1];
  for(int i=0;i<6;i++) {
    judge(poly,x,y,z, gb, view_window[i]);
    //chop
    for(j=1;j<=l;j++) {
      int nj = (j+1)%(l+1) + 1;
      if(gb[j]==0) {
	if(gb[nj]==0){
	  ...
	  //add the intersection point with current plane and line 
	  //between current and next point
	}
      } else {
	if(gb[nj] == 0) {
	  //add the intersection between line for current and next point and plane
	} else {
	  //next point
	}
      }
...
void draw (double *x, double *y,double *z,int p, int *poly) {
  ...
  for(j=1;j<=poly[0];j++) {
    i = poly[j];
    temp_x[j-1] = x[i]*(window_width/2) / (z[i] * tan(half_angle)) + window_width/2;
    temp_y[j-1] =  y[i]*(window_width/2) / (z[i] * tan(half_angle)) + window_height/2;
  }
...
int main(int argc, char **argv) {
  ...
  double x[10][5000], y[10][5000], z[10][5000], x2d[10][5000], y2d[10][5000];
