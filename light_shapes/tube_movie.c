           ...
           double rgb[3])
{
  ...
  checker(RGB,black,AMBIENT+DIFFUSE_MAX, AMBIENT + diffuse + specular, u, v, rgb);
}
...
int main()
{
  ...
  for(u=0;u<2*M_PI;u+=M_PI/50){
    ...
    plot_3d_with_inc(map,0.0, 2*M_PI, 0,2*M_PI, f8, mat, rgb,0.05,0.05);
    ...
    plot_3d_with_inc(map, -v, v, 0, 2*M_PI, f9, mat, rgb,0.02,0.02);
