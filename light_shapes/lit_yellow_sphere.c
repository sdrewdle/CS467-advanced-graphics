	   ...
	   int (*func)(double uu, double vv, double xyz[3]))
{
  ...
  func(u+0.001,v,a);   D3d_mat_mult_pt(a,mat,a);
  func(u,v+0.001,c);   D3d_mat_mult_pt(c,mat,c);
  normal(a,b,c,norm);

  //------------------------define the eye vector and the light source vector
  double eyev[3], lsv[3];
  vector_to(eye,b,eyev); vector_to(ls,b,lsv);
  normalize(eyev,3);
  normalize(lsv,3);
  
  double den = dot(eyev,norm), dln = dot(lsv,norm);
  ...
  double specular, diffuse; //constants above: diffuse_max, spec_pow, ambient
  
  double rv[3];
  reflection(lsv,norm,rv);
  
  double cosA = dot(eyev,rv);
  if(cosA < 0) {
    specular = 0;
  } else {
    specular = (1-ambient-diffuse_max) * pow(cosA, spec_pow);
  }

  double cosB = dot(norm,lsv);
  if(cosB < 0) {
    diffuse = 0;
  } else {
    diffuse = diffuse_max * cosB;
  }
...
int main()
{
  ...
  Ttypelist[Tn] = TX ; Tvlist[Tn] =  1.5 ; Tn++ ; //2.5
  Ttypelist[Tn] = TY ; Tvlist[Tn] =  0.0 ; Tn++ ; //.5
  Ttypelist[Tn] = TZ ; Tvlist[Tn] =  4 ; Tn++ ; //7
